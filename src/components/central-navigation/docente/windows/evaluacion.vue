<template>
    <div class="evaluacion">
        ...
        <v-container>
            <v-row>
                <v-col cols="12">
                <h1 class="titulo-seccion">EVALUANDO A: {{grupoSelected}}
                </h1>
                    ...
                    <v-col cols="12" md="12">
                        <template>
                            <div class="tabla-avisos">
                                ...
                                height="303"
                                >
                                <template v-slot:[`item.calificaciones`]="{ item }" ><!--HELP MINERO-->
                                    <v-btn
                                    color="#a19405" 
                                    outlined
                                ...
                                <template v-slot:[`item.observa_nov`]="{ item }" ><!--HELP MINERO-->
                                    <v-btn 
                                    color="#23a894"
                                    outlined
                                ...
                                <template v-slot:[`item.observa_mar`]="{ item }" ><!--HELP MINERO-->
                                    <v-btn 
                                    color="#0978b8"
                                    outlined
                                    @click="evaluarMarzo(item.id_alumno)"
                                    >
                                        <v-icon
                                        color="#0978b8"
                                        class="mx-1"
                                ...
                                <template v-slot:[`item.observa_jun`]="{ item }" ><!--HELP MINERO-->
                                    <v-btn 
                                    color="#7209b8"
                                    outlined
                                    @click="evaluarJunio(item.id_alumno)"
                                    >
                                        <v-icon
                                        color="#7209b8"
                                        class="mx-1"
                                ...
                                <template v-slot:[`item.actions`]="{ item2 }"
                                    ><!--HELP MINERO-->
                                    <v-icon
                                    @click="EliminarNoticia(item2.id_alumno)"
                                    color="#F97068"
                                    >
                                    mdi-delete-forever
                                    </v-icon>
        ...
        <revisaCalificacion :alumnoId="alumnoSelected" :alumnoName="alumnoSelectedName" :calificacionesData = "dataCalificaciones" />
        <evaluarNoviembre :alumnoId="alumnoSelected" />
    ...
    data(){
        return{
            ...
            encabezadosTabla: [
                ...
                { text: "Observaciones Noviembre", value: "observa_nov", sortable: false },
        ...
        async obtenerAlumnos(id_grupo){
            ...
            .then((r) => {
                ...
                this.DatosAlumnos = r.data.map((a )=>{
                    ...
                    .where('id_alumno', '==', a.id_alumno ).get()
                    .then((snapshot) => {
